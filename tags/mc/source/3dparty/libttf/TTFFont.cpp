...
vec2s Font::GetKerning (uint16_t g0, uint16_t g1, bool hk) const {
	...
	// intialize variables
	vec2s kv = vec2s(static_cast<int16_t>(GetGlyphMetrics(g0).advance_width),0);		// get default advance width
...
FontMetrics Font::GetFontMetrics () const {
	...
	fm.caret_slope = vec2s(caret_slope_run,caret_slope_rise);
...
VFontMetrics Font::GetVFontMetrics () const {
	...
	vfm.caret_slope = vec2s(caret_slope_run,caret_slope_rise);
...
vec4s Font::GetGlyphRect (uint16_t glyph_index) const {
	...
	vec4s r(ReadSHORT(itr),ReadSHORT(itr),ReadSHORT(itr),ReadSHORT(itr));
